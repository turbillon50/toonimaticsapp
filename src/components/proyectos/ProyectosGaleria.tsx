...
import ProyectoCard, { type ProyectoEstado } from '@/components/proyectos/ProyectoCard'
...
export default function ProyectosGaleria({ proyectos }: ProyectosGaleriaProps) {
  const [filtro, setFiltro] = useState<FiltroEstado>('todos')
  const [busqueda, setBusqueda] = useState('')
  const [favoritos, setFavoritos] = useState<string[]>([])

  const proyectosConFavoritos = useMemo(
    () =>
  ...
  const proyectosFiltrados = useMemo(() => {
    ...
    return proyectosConFavoritos.filter((proyecto) => {
  ...
  }, [busqueda, filtro, proyectosConFavoritos])
  ...
  const etiquetas = getEtiquetas(proyectos.length, enProceso, terminados)
...
function getEtiquetas(total: number, enProceso: number, terminados: number) {
  const etiquetas = [
    { id: 'total', label: `${total} proyectos`, color: '#FF6B00' },
  ]

  if (enProceso > 0) {
    etiquetas.push({ id: 'en-proceso', label: 'Produccion activa', color: '#F2C53D' })
  }

  if (terminados > 0) {
    etiquetas.push({ id: 'terminados', label: 'Obras terminadas', color: '#3CA55C' })
  }
